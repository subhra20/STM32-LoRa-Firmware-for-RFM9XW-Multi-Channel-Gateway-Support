...
/* ----------------------------------------------------------------------------- *\
		name        : LoRa_setSpreadingFactor_ii
...
/* ----------------------------------------------------------------------------- *\
		name        : LoRa_setTOMsb_setCRCon_ii
...
uint8_t LoRa_transmit_ii(LoRa_ii* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout){
	...
	read = LoRa_read_ii(_LoRa, RegFiFoTxBaseAddr);
	LoRa_write_ii(_LoRa, RegFiFoAddPtr, read);
	LoRa_write_ii(_LoRa, RegPayloadLength, length);
	LoRa_BurstWrite_ii(_LoRa, RegFiFo, data, length);
	LoRa_gotoMode_ii(_LoRa, TRANSMIT_MODE);
	while(1){
		read = LoRa_read_ii(_LoRa, RegIrqFlags);
		if((read & 0x08)!=0){
			LoRa_write_ii(_LoRa, RegIrqFlags, 0xFF);
			LoRa_gotoMode_ii(_LoRa, mode);
			return 1;
		}
		else{
			if(--timeout==0){
...
int LoRa_getRSSI_ii(LoRa_ii* _LoRa){
	uint8_t read;
	read = LoRa_read_ii(_LoRa, RegPktRssiValue);
	return -164 + read;
}
