...
/* ----------------------------------------------------------------------------- *\
		...
		returns     : Nothing
\* ----------------------------------------------------------------------------- */
void LoRa_reset_iii(LoRa_iii* _LoRa){
...
/* ----------------------------------------------------------------------------- *\
		name        : LoRa_setSpreadingFactor_iii
...
/* ----------------------------------------------------------------------------- *\
		name        : LoRa_setTOMsb_setCRCon_iii
...
/* ----------------------------------------------------------------------------- *\
		...
		returns     : Nothing
\* ----------------------------------------------------------------------------- */
void LoRa_BurstWrite_iii(LoRa_iii* _LoRa, uint8_t address, uint8_t *value, uint8_t length){
	uint8_t addr;
	addr = address | 0x80;
...
\* ----------------------------------------------------------------------------- */
uint8_t LoRa_transmit_iii(LoRa_iii* _LoRa, uint8_t* data, uint8_t length, uint16_t timeout){
	uint8_t read;

	int mode = _LoRa->current_mode;
	LoRa_gotoMode_iii(_LoRa, STNBY_MODE);
	read = LoRa_read_iii(_LoRa, RegFiFoTxBaseAddr);
	LoRa_write_iii(_LoRa, RegFiFoAddPtr, read);
	LoRa_write_iii(_LoRa, RegPayloadLength, length);
	LoRa_BurstWrite_iii(_LoRa, RegFiFo, data, length);
	LoRa_gotoMode_iii(_LoRa, TRANSMIT_MODE);
	while(1){
		read = LoRa_read_iii(_LoRa, RegIrqFlags);
		if((read & 0x08)!=0){
			LoRa_write_iii(_LoRa, RegIrqFlags, 0xFF);
			LoRa_gotoMode_iii(_LoRa, mode);
			return 1;
		}
		else{
			if(--timeout==0){
...
\* ----------------------------------------------------------------------------- */
int LoRa_getRSSI_iii(LoRa_iii* _LoRa){
	uint8_t read;
	read = LoRa_read_iii(_LoRa, RegPktRssiValue);
	return -164 + read;
}
